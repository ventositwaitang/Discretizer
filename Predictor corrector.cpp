...
void simulateCIR(const double& alpha, const double& beta, const double& sigma,
    ...
    for (int i = 1; i <= numSteps; ++i) {
        ...
        interestRate[i] = std::max(0.0, interestRate[i - 1] + beta * (alpha - interestRate[i - 1]) * dt
                                           + sigma * std::sqrt(std::max(0.0, interestRate[i - 1])) * std::sqrt(dt) * dW); // Euler-Maruyama method
        interestRate[i] = std::max(0.0, interestRate[i] + 1/2 * (beta * (alpha - interestRate[i]) - beta * (alpha - interestRate[i-1])) * dt); // Corrector of Euler-Maruyama method
    }
